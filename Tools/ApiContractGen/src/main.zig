...
const phase_a_groups = [_]ExpectedGroup{
    ...
    .{ .id = 1, .name = "R4SYS", .kind = .kernel_table, .functions = 115, .reserved = 2, .tombstones = 1 },
    ...
    // R4DEV v5 appends slot 35 for the running kernel version while slot 27 remains the frozen storage v1 prefix.
    .{ .id = 6, .name = "R4DEV", .kind = .kernel_table, .functions = 34, .reserved = 2, .tombstones = 0 },
};
